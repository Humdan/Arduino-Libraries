...
// Constructor, to which you pass a previously generated arduino  Object 
// e.g. - 
//   arduino NSS(0,2);   // Note that SerLCD only has Rx and no Tx, so only really need a Tx from Arduino
//   SerLCD theLCD(NSS); 
// This will default to 16 columns, 2 row LCD.  Use the other constructor for more options. 
SerLCD::SerLCD(arduino &_SerTX)
{
	SerTX = &_SerTX; 
	columns=16;
	rows=2;
...
SerLCD::SerLCD(arduino &_SerTX, int _columns, int _rows)
{
	SerTX = &_SerTX; 
	columns=_columns;
	rows=_rows; 
